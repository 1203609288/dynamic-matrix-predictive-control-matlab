...
a_gp = exp(-h_gp / T0_gp); b_gp = k0_gp*(1-a_gp);%系数矩阵
...
DMC_y0	= zeros( m_nHP+1, 1 );			% 无 deltaU(k) 作用时的自由响应, y0(k+i/k).		计算出deltaU(k)后,则被更新,相当于 y1(k+i/k) = y0(k+i/k+1)
...
DMC_y0_mem = zeros(itEnd, m_nHP+1);
DMC_y01_mem =zeros(itEnd, m_nHP+1);		%计算出deltaU(k)后,则被更新,相当于 y1(k+i/k) = y0(k+i/k+1)
...
while (it < itEnd)
	...
	if( mod(it, 4) == 1 )	
		...
		if( itdmc == 1 )		% 第一次执行DMC控制
			...
			DMC_y0 ( 2 : m_nHP+1 )   = DMC_y0 ( 2 : m_nHP+1 ) + yerror;	% 反馈校正
			%DMC_y0 ( 1 : m_nHP ) = DMC_y0 ( 2 : m_nHP+1 );				%滚动,与没有修正的完全一样
			DMC_y0 ( 1 : m_nHP ) = DMC_y0 ( 2 : m_nHP+1 );				% 滚动
...
%			end
		...
		% 下式没有前馈补偿
		DMC_ye = DMC_yw - DMC_y0 ( 1 : m_nHP ) ;		% 设定值-预测y		
        ...
        DMC_y0 = DMC_y0 	 + duk * y_dmc_step(1 : m_nHP+1); 		% 计算出 y1		
...
figure();
msg = sprintf('没有前馈补偿的DMC Controler: HP=%d, HM=%d, A(%d,1)=%f. LRJ修正41', m_nHP, m_nHM, m_nHP,A(m_nHP,1));
